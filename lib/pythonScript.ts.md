...
Builds the Python source code string that reconstruct the definition of an optical system for RayOptics inside the Pyodide worker. It is also for UI components to let users to copy the Python snippet to the clipboard so that users may use the code string for their own RayOptics instance on Jupyter notebook.

## Exports

```ts
export function buildOpticalModelScript(
...
### `buildOpticalModelScript(model, setAutoAperture)`
- This function is also used in the Pyodide worker.
...
4. Sets `opm.radius_mode = True`. Sets `sm.do_apertures` based on a flag.
...
### `buildExportScript(model, setAutoAperture)`
...
2. The full output of `buildOpticalModelScript(model, setAutoAperture)`.
...
- `CaF2` medium is emitted as the bare variable `caf2` (no quotes); `buildExportScript` provides the `caf2` binding in its preamble. Callers using `buildOpticalModelScript` alone in the worker must ensure `caf2` is already defined.
...
- `buildOpticalModelScript` is called inside `workers/pyodide.worker.ts` via `runPython` / `setOpticalSurfaces` to send a new model to the Pyodide interpreter.
