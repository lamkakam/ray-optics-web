...
export function SideNav({ isOpen, isLG, currentView, onClose, onNavigate }: SideNavProps) {
  ...
  return (
    <nav
      ...
      className={`absolute top-0 left-0 h-full z-40 flex flex-col bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 shadow-xl ${isLG ? "w-[33vw]" : "w-[50vw]"}`}
    >
      ...
      <div className="flex flex-col gap-1 px-2">
        {NAV_ITEMS.map(({ view, label }) => (
          <Button
            key={view}
            variant={currentView === view ? "toggle" : "secondary"}
            aria-label={label}
