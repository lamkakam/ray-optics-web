...
| `getDiffractionPSFData(model, fi, wi, imagePoint?, numRays?, maxDims?)` | Returns `DiffractionPsfData` for the given field and wavelength index using `json.dumps(get_diffraction_psf_data(...))`. Used by the deck.gl Diffraction PSF view. |
