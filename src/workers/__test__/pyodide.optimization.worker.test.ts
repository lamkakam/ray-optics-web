...
describe("_optimizeGlasses", () => {
  ...
  it("serializes the flat glass config and invokes optimize_glasses", async () => {
    const runPython = jest
      .fn()
      .mockResolvedValue(JSON.stringify(glassReport()));
    ...
    expect(source).toContain(
      '\\"glass_optimizer\\":{\\"num_neighbours\\":2,\\"maxiter\\":20,\\"tol\\":0.0001}',
    );
    expect(source).toContain(
  ...
  it("parses candidate-aware progress through the callback registered in Python globals", async () => {
    ...
    const runPython = jest.fn().mockImplementation(async () => {
      const boundCallback = pyodideGlobalsSet.mock.calls[0]?.[1] as
        | ((json: string) => void)
        | undefined;
    ...
    expect(pyodideGlobalsSet).toHaveBeenCalledWith(
  ...
  it("is present on the typed public worker API", () => {
    const optimizeGlasses: PyodideWorkerAPI["optimizeGlasses"] = async () =>
      glassReport();
...
describe("_optimizeOpm", () => {
  ...
  it("runs optimize_opm in Python and parses the JSON result", async () => {
    ...
    expect(runPython).toHaveBeenCalledWith(
      expect.stringContaining("image_point='chief_ray'"),
    );
    expect(runPython).toHaveBeenCalledWith(
      expect.not.stringContaining("candidate_materials="),
    );
    expect(runPython).toHaveBeenCalledWith(
    ...
    expect(runPython).toHaveBeenCalledWith(
      expect.stringContaining("optimize_opm("),
    );
    expect(runPython).toHaveBeenCalledWith(
      expect.stringContaining('\\"kind\\":\\"least_squares\\"'),
    );
  });
  ...
  it("streams optimization progress through the supplied callback and returns the final history", async () => {
    ...
    const runPython = jest.fn().mockImplementation(async () => {
      const registeredCallback = globalsSet.mock.calls[0]?.[1] as
        | ((json: string) => void)
        | undefined;
      registeredCallback?.(
        JSON.stringify([
          {
            iteration: 0,
            merit_function_value: 25,
            log10_merit_function_value: Math.log10(25),
          },
        ]),
      );
      registeredCallback?.(
    ...
    const result = await _optimizeOpm(
      runPython,
      baseModel,
      {
        optimizer: {
    ...
    expect(runPython.mock.calls[0]?.[0]).toContain(
      "progress_reporter=_report_optimization_progress",
    );
  ...
  it("omits progress binding when the callback is absent", async () => {
    ...
    const runPython = jest
      .fn()
      .mockResolvedValue(JSON.stringify(optimizationErrorReport()));
  ...
  it.each([
    ...
    "missing globals",
    "missing globals.set",
    "missing globals.delete",
  ] as const)("omits progress binding with %s", async (missingPart) => {
    const globalsSet = jest.fn();
    const globalsDelete = jest.fn();
    const runtime =
  ...
  it.each(["success", "failure"] as const)(
    ...
    async (outcome) => {
      const progressCallback = jest.fn() as jest.Mock & {
        [releaseProxy]?: () => void;
      };
  ...
  it.each([
    ...
    const interruptBuffer =
      missingPart === "missing interrupt buffer"
        ? undefined
  ...
  it("signals only the active matching run id and treats stale stops as no-ops", async () => {
    ...
    const runPython = jest.fn().mockImplementation(async () => {
      expect(await _requestOptimizationStop("old-run")).toEqual({
        signaled: false,
      });
      expect(Atomics.load(interruptView, 0)).toBe(0);
      expect(await _requestOptimizationStop("run-2")).toEqual({
        signaled: true,
      });
      expect(Atomics.load(interruptView, 0)).toBe(2);
      return JSON.stringify({
...
describe("_evaluateOptimizationProblem", () => {
  it("runs evaluate_optimization_problem in Python and parses the JSON result", async () => {
    ...
    expect(runPython).toHaveBeenCalledWith(
      expect.stringContaining("evaluate_optimization_problem("),
    );
    expect(runPython).toHaveBeenCalledWith(
      expect.stringContaining('\\"kind\\":\\"least_squares\\"'),
    );
  ...
  it("passes ray_fan operands without target and accepts residuals without target", async () => {
    ...
    expect(runPython).toHaveBeenCalledWith(
      expect.stringContaining('\\"kind\\":\\"ray_fan\\"'),
    );
    expect(runPython).toHaveBeenCalledWith(
      expect.not.stringContaining('\\"target\\"'),
    );
  });
  ...
  it("preserves image-surface radius variables in the generated Python config", async () => {
    ...
    expect(runPython).toHaveBeenCalledWith(
      expect.stringContaining('\\"surface_index\\":2'),
    );
  });
  ...
  it("preserves asphere optimization metadata in the generated Python config", async () => {
    ...
    await _evaluateOptimizationProblem(
      runPython,
      {
        ...baseModel,
        surfaces: [
    ...
    expect(runPython).toHaveBeenCalledWith(
      expect.stringContaining('\\"asphere_kind\\":\\"RadialPolynomial\\"'),
    );
    expect(runPython).toHaveBeenCalledWith(
      expect.stringContaining('\\"coefficient_index\\":1'),
    );
  });
});
