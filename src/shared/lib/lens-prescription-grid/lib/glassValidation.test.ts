...
function makeLookupMaps(keys: readonly string[]): GlassLookupMaps {
  return {
    ...
    mediumMap: new Map(
      keys.map((key) => [key, { medium: key, manufacturer: "" }]),
    ),
...
function modelWithMedia(
  media: ReadonlyArray<readonly [string, string]>,
): OpticalModel {
  return {
    ...baseModel,
...
describe("glassValidation", () => {
  it.each([
    [
      { medium: " AIR ", manufacturer: "anything" },
    ...
    [
      { medium: " 1.458 ", manufacturer: " 51.38 " },
      { medium: "1.458", manufacturer: "51.38" },
    ],
  ])("canonicalizes lookup-independent medium %#", (input, expected) => {
    expect(resolvePrescriptionMedium(input, undefined)).toEqual({
      kind: "resolved",
      value: expected,
    });
  });
  ...
  it("does not treat whitespace-only media as numeric glass", () => {
    expect(
      resolvePrescriptionMedium({ medium: "   ", manufacturer: "" }, undefined),
    ).toEqual({
  ...
  it("resolves Special, catalog, and Custom media in precedence order", () => {
    ...
    expect(
      resolvePrescriptionMedium(
        { medium: "water", manufacturer: "Schott" },
        maps,
      ),
    ).toEqual({
      kind: "resolved",
      value: { medium: "Water", manufacturer: "" },
    });
    expect(
      resolvePrescriptionMedium(
      ...
      kind: "resolved",
      value: { medium: "N-BK7", manufacturer: "Schott" },
    });
    expect(
      resolvePrescriptionMedium(
        { medium: "USER GLASS", manufacturer: "wrong" },
        maps,
      ),
    ).toEqual({
      kind: "resolved",
      value: { medium: "User Glass", manufacturer: "Custom" },
    });
  });
  ...
  it("canonicalizes a whole prescription and reports all unknown rows with JSON-pointer paths", () => {
    ...
    expect(
      resolvePrescriptionMedia(modelWithMedia([["n-bk7", "SCHOTT"]]), maps),
    ).toEqual({
      kind: "resolved",
  ...
  it("reports an unknown Object medium at the Object JSON-pointer path", () => {
    ...
    const model = {
      ...modelWithMedia([]),
      object: { distance: 10, medium: "  Missing ", manufacturer: " Hoya " },
    };
    ...
    expect(getMissingPrescriptionGlasses(model, maps)).toEqual([
      "Hoya: Missing",
    ]);
  });
  ...
  it("formats the default validation error message", () => {
    expect(
      formatMissingGlassMessage(["Schott: N-BK7", "Custom: MY_GLASS"]),
    ).toBe(
