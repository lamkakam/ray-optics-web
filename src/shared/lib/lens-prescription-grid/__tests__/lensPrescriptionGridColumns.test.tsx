...
describe("lens prescription grid columns", () => {
  it("creates an explicitly text-edited comment column for physical surfaces only", () => {
    ...
    if (
      ...
      typeof column.valueSetter !== "function"
    )
      return;

    expect(
      column.valueGetter({ data: physicalRow } as ValueGetterParams<GridRow>),
    ).toBe("Front element");
    expect(
      column.valueGetter({
        data: { ...physicalRow, comment: undefined },
      } as ValueGetterParams<GridRow>),
    ).toBe("");
    expect(
      column.valueGetter({ data: objectRow } as ValueGetterParams<GridRow>),
    ).toBe("");
    expect(
      column.valueGetter({ data: imageRow } as ValueGetterParams<GridRow>),
    ).toBe("");
    ...
    expect(
      column.valueSetter({ data: physicalRow, newValue: "Updated" } as never),
    ).toBe(true);
  ...
  it("blanks and disables semi-diameter for rectangular clear apertures", () => {
    ...
    const column = createSemiDiameterColumn({
      getGridRow,
    ...
    expect(
      column.valueGetter({ data: row } as ValueGetterParams<GridRow>),
    ).toBeUndefined();
    expect(column.editable({ data: row } as never)).toBe(false);
  });
  ...
  it("keeps shared grid defaults non-sortable and immovable", () => {
    expect(lensPrescriptionGridDefaultColDef).toEqual({
      sortable: false,
  ...
  it("keeps the common column order stable for the lens editor", () => {
    expect(
      createLensPrescriptionCommonColumns({ getGridRow }).map(
  ...
  it.each([
    ...
    "Infinity",
    "NaN",
  ])("preserves the old value for invalid numeric input %s", (newValue) => {
  ...
  it("returns the expected value for every row kind in the display columns", () => {
    const value = (
      column: { valueGetter?: unknown },
      data: GridRow | undefined,
    ) => {
      if (typeof column.valueGetter !== "function")
        throw new Error("Expected a value getter");
      return column.valueGetter({ data } as ValueGetterParams<GridRow>);
    };

    expect(value(createSurfaceColumn({ getGridRow }), rows.object)).toBe(
      "Object",
    );
    expect(value(createSurfaceColumn({ getGridRow }), rows.image)).toBe(
      "Image",
    );
    expect(value(createSurfaceColumn({ getGridRow }), rows.surface)).toBe(
      "Stop",
    );
    expect(value(createSurfaceColumn({ getGridRow }), undefined)).toBe("");

    expect(value(createCommentColumn({ getGridRow }), rows.surface)).toBe(
      "Front element",
    );
    ...
    expect(
      value(createRadiusOfCurvatureColumn({ getGridRow }), rows.object),
    ).toBeUndefined();
    expect(
      value(createRadiusOfCurvatureColumn({ getGridRow }), rows.image),
    ).toBe(-8);
    expect(
      value(createRadiusOfCurvatureColumn({ getGridRow }), rows.surface),
    ).toBe(12);
    expect(
      value(createRadiusOfCurvatureColumn({ getGridRow }), undefined),
    ).toBeUndefined();

    expect(value(createThicknessColumn({ getGridRow }), rows.object)).toBe(
      1e10,
    );
    expect(
      value(createThicknessColumn({ getGridRow }), rows.image),
    ).toBeUndefined();
    expect(value(createThicknessColumn({ getGridRow }), rows.surface)).toBe(2);
    expect(
      value(createThicknessColumn({ getGridRow }), undefined),
    ).toBeUndefined();

    expect(value(createMediumColumn({ getGridRow }), rows.object)).toBe("air");
    expect(
      value(createMediumColumn({ getGridRow }), rows.image),
    ).toBeUndefined();
    expect(value(createMediumColumn({ getGridRow }), rows.surface)).toBe(
      "N-BK7",
    );
    expect(
      value(createMediumColumn({ getGridRow }), undefined),
    ).toBeUndefined();
    ...
    expect(
      value(createApertureColumn({ getGridRow }), rows.object),
    ).toBeUndefined();
    expect(
      value(createApertureColumn({ getGridRow }), rows.image),
    ).toBeUndefined();
    expect(
      value(createApertureColumn({ getGridRow }), undefined),
    ).toBeUndefined();
    expect(
      value(createAsphericalColumn({ getGridRow }), undefined),
    ).toBeUndefined();
    expect(
      value(createAsphericalColumn({ getGridRow }), rows.object),
    ).toBeUndefined();
    expect(
      value(createAsphericalColumn({ getGridRow }), rows.image),
    ).toBeUndefined();
    expect(value(createAsphericalColumn({ getGridRow }), rows.surface)).toEqual(
      rows.surface.aspherical,
    );
    expect(
      value(createDecenterColumn({ getGridRow }), rows.object),
    ).toBeUndefined();
    expect(value(createDecenterColumn({ getGridRow }), rows.image)).toEqual(
      rows.image.decenter,
    );
    expect(value(createDecenterColumn({ getGridRow }), rows.surface)).toEqual(
      rows.surface.decenter,
    );
    expect(
      value(createDecenterColumn({ getGridRow }), undefined),
    ).toBeUndefined();
    expect(
      value(createDiffractionGratingColumn({ getGridRow }), rows.object),
    ).toBeUndefined();
    expect(
      value(createDiffractionGratingColumn({ getGridRow }), rows.image),
    ).toBeUndefined();
    expect(
      value(createDiffractionGratingColumn({ getGridRow }), rows.surface),
    ).toEqual(rows.surface.diffractiveElement?.diffractionGrating);
    expect(
      value(createDiffractionGratingColumn({ getGridRow }), undefined),
    ).toBeUndefined();

    const surfaceWithoutGrating = {
      ...rows.surface,
      diffractiveElement: undefined,
    } satisfies GridRow;
    expect(
      value(
        createDiffractionGratingColumn({ getGridRow }),
        surfaceWithoutGrating,
      ),
    ).toBeUndefined();
  });
  ...
  it("enables only supported editable row kinds and forwards edit callbacks", () => {
    ...
    expect(invokeColumnCallback(surface.editable, params(rows.surface))).toBe(
      true,
    );
    expect(invokeColumnCallback(surface.editable, params(rows.object))).toBe(
      false,
    );
    expect(invokeColumnCallback(surface.editable, params(undefined))).toBe(
      false,
    );
    expect(
      invokeColumnCallback(surface.valueSetter, {
        data: rows.surface,
        newValue: "Default",
      }),
    ).toBe(true);
    ...
    expect(
      invokeColumnCallback(
        createSurfaceColumn({ getGridRow }).editable,
        params(rows.surface),
      ),
    ).toBe(false);
    expect(
      invokeColumnCallback(createSurfaceColumn({ getGridRow }).valueSetter, {
        data: rows.surface,
        newValue: "Default",
      }),
    ).toBe(false);
    ...
    expect(invokeColumnCallback(comment.editable, params(rows.surface))).toBe(
      true,
    );
    expect(invokeColumnCallback(comment.editable, params(rows.object))).toBe(
      false,
    );
    expect(
      invokeColumnCallback(comment.valueSetter, {
        data: rows.object,
        newValue: "ignored",
      }),
    ).toBe(false);
    ...
    expect(invokeColumnCallback(comment.editable, params(undefined))).toBe(
      false,
    );
    expect(
      invokeColumnCallback(createCommentColumn({ getGridRow }).valueSetter, {
        data: rows.surface,
        newValue: "ignored",
      }),
    ).toBe(false);

    const radius = createRadiusOfCurvatureColumn({
      getGridRow,
      onRadiusChange,
    });
    expect(invokeColumnCallback(radius.editable, params(rows.surface))).toBe(
      true,
    );
    expect(invokeColumnCallback(radius.editable, params(rows.object))).toBe(
      false,
    );
    expect(invokeColumnCallback(radius.editable, params(undefined))).toBe(
      false,
    );
    expect(
      invokeColumnCallback(radius.valueSetter, {
        data: rows.surface,
        newValue: 18,
      }),
    ).toBe(true);
    ...
    expect(
      invokeColumnCallback(
        createRadiusOfCurvatureColumn({ getGridRow }).editable,
        params(rows.image),
      ),
    ).toBe(false);
    expect(
      invokeColumnCallback(
        createRadiusOfCurvatureColumn({ getGridRow }).valueSetter,
        { data: rows.surface, newValue: 18 },
      ),
    ).toBe(false);

    const thickness = createThicknessColumn({ getGridRow, onThicknessChange });
    expect(invokeColumnCallback(thickness.editable, params(rows.object))).toBe(
      true,
    );
    expect(invokeColumnCallback(thickness.editable, params(rows.surface))).toBe(
      true,
    );
    expect(invokeColumnCallback(thickness.editable, params(rows.image))).toBe(
      false,
    );
    expect(invokeColumnCallback(thickness.editable, params(undefined))).toBe(
      false,
    );
    expect(
      invokeColumnCallback(thickness.valueSetter, {
        data: rows.surface,
        newValue: 9,
      }),
    ).toBe(true);
    ...
    expect(
      invokeColumnCallback(
        createThicknessColumn({ getGridRow }).editable,
        params(rows.surface),
      ),
    ).toBe(false);
    expect(
      invokeColumnCallback(createThicknessColumn({ getGridRow }).valueSetter, {
        data: rows.surface,
        newValue: 9,
      }),
    ).toBe(false);

    const semiDiameter = createSemiDiameterColumn({
      getGridRow,
      onSemiDiameterChange,
    });
    expect(
      invokeColumnCallback(semiDiameter.editable, params(rows.surface)),
    ).toBe(true);
    expect(
      invokeColumnCallback(semiDiameter.editable, params(rows.object)),
    ).toBe(false);
    expect(
      invokeColumnCallback(semiDiameter.editable, params(rows.image)),
    ).toBe(false);
    expect(invokeColumnCallback(semiDiameter.editable, params(undefined))).toBe(
      false,
    );
    expect(
      invokeColumnCallback(semiDiameter.valueSetter, {
        data: rows.surface,
        newValue: 6,
      }),
    ).toBe(true);
    ...
    expect(
      invokeColumnCallback(
        createSemiDiameterColumn({ getGridRow }).editable,
        params(rows.surface),
      ),
    ).toBe(false);
    expect(
    ...
    expect(createSurfaceColumn({ getGridRow }).cellEditorParams).toEqual({
      values: ["Default", "Stop"],
    });
  });
  ...
  it("applies the semi-diameter style for manual, automatic, rectangular, and missing rows", () => {
    ...
    const automatic = createSemiDiameterColumn({
      getGridRow,
      semiDiameterReadonly: true,
    });
    const style = (
      column: { cellStyle?: unknown },
      data: GridRow | undefined,
    ) => {
      if (typeof column.cellStyle !== "function")
        throw new Error("Expected a cell style callback");
    ...
    expect(
      invokeColumnCallback(computedObjectValue.valueGetter, {
        data: rows.object,
      }),
    ).toBeUndefined();
  });
  ...
  it("renders action cells only for supported rows and invokes each modal callback", () => {
    ...
    for (const action of actions) {
      ...
      expect(
        column.cellRenderer?.({ data: undefined } as never),
      ).toBeUndefined();
      expect(
        column.cellRenderer?.({ data: action.unsupported } as never),
      ).toBeUndefined();
      expect(
        action
          .create(action.options as never)
          .cellRenderer?.({ data: action.row } as never),
      ).toBeUndefined();
