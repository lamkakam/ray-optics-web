...
describe("validateLensPrescription", () => {
  ...
  it.each([
    ...
    [
      "reflective object medium",
      { ...prescription, object: { ...prescription.object, medium: "REFL" } },
    ],
    [
      "non-finite number",
      { ...prescription, image: { curvatureRadius: Infinity } },
    ],
    [
      "bad asphere discriminator",
      {
        ...prescription,
        surfaces: [
          {
            ...prescription.surfaces[0],
            aspherical: { kind: "Odd", conicConstant: 0 },
          },
        ],
      },
    ],
        ...
        surfaces: [
          {
            ...
            aspherical: {
              kind: "EvenAspherical",
              conicConstant: 0,
              polynomialCoefficients: Array(11).fill(0),
            },
          },
        ],
      },
    ...
    [
      "bad decenter strategy",
      {
        ...prescription,
        image: {
          curvatureRadius: 0,
          decenter: {
            coordinateSystemStrategy: "move",
            alpha: 0,
            beta: 0,
            gamma: 0,
            offsetX: 0,
    ...
    [
      "non-positive rectangle dimension",
      {
        ...prescription,
        surfaces: [
          {
        ...
        surfaces: [
          {
            ...prescription.surfaces[0],
            diffractiveElement: {
              diffractionGrating: { lpmm: 1200, order: 1.5 },
            },
          },
        ],
      },
  ...
  it.each([
    ...
    "manufacturer",
    "semiDiameter",
  ])("requires every physical-surface field: %s", (field) => {
    const surface = { ...prescription.surfaces[0] } as Record<string, unknown>;
    delete surface[field];

    expect(
      validateLensPrescription({ ...prescription, surfaces: [surface] }),
    ).toBe(false);
  });
    ...
    expect(
      validateLensPrescription({
        ...
        surfaces: [
          {
            ...prescription.surfaces[0],
            decenter: { ...prescription.surfaces[0].decenter, extra: true },
          },
        ],
      }),
    ).toBe(false);
        ...
        surfaces: [
          {
            ...prescription.surfaces[0],
            clear_aperture: {
              ...prescription.surfaces[0].clear_aperture,
              extra: true,
            },
          },
        ],
  ...
  it.each(["bend", "dec and return", "decenter", "reverse"])(
    ...
    (coordinateSystemStrategy) => {
      const decenter = {
        ...prescription.surfaces[0].decenter,
        coordinateSystemStrategy,
      };
    ...
    expect(
      validateLensPrescription({
        ...
        surfaces: [
          {
            ...prescription.surfaces[0],
            clear_aperture: { shape: "ellipse", offsetX: 0, offsetY: 0 },
          },
        ],
      }),
    ).toBe(false);
