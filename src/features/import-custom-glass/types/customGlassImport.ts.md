...
- `ModalMode` and `ConfirmationMode` enumerate page-level modal states.
- `UserDefinedCustomCatalog`, `ImportedCustomGlassMaterial`, and `RejectedCsvFile` model custom catalog maps and import results.
- `CustomGlassStoreActions` and `SaveCustomGlassOptions` keep worker orchestration loosely coupled from the Glass Map Zustand store.
