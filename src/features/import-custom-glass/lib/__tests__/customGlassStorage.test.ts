...
const secondInput: UserDefinedGlassInput = {
  name: "SECOND",
  pairs: [[546.07, 1.518], [656.27, 1.514]],
};

function openRawDatabase(factory: IDBFactory): Promise<IDBDatabase> {
  const request = factory.open(customGlassStorageConstants.dbName, customGlassStorageConstants.dbVersion);
...
async function readRawStore(factory: IDBFactory, storeName: string): Promise<readonly unknown[]> {
...
async function writeRawRows(factory: IDBFactory, rows: readonly unknown[]): Promise<void> {
  const db = await openRawDatabase(factory);
  try {
    const transaction = db.transaction(customGlassStorageConstants.customStore, "readwrite");
    const store = transaction.objectStore(customGlassStorageConstants.customStore);
    for (const row of rows) {
      store.put(row);
    }
    await new Promise<void>((resolve, reject) => {
      transaction.oncomplete = () => resolve();
      transaction.onerror = () => reject(transaction.error ?? new Error("Failed to write test rows."));
      transaction.onabort = () => reject(transaction.error ?? new Error("Test row transaction aborted."));
    });
  } finally {
    db.close();
  }
  ...
  try {
    return await Promise.race([
      ...
      new Promise<T>((_resolve, reject) => {
        timeout = setTimeout(() => reject(new Error("IndexedDB test operation did not settle.")), 250);
      }),
    ]);
  } finally {
...
function makeFailureFactory(failure: Failure): IDBFactory {
      ...
      getAll: () => {
        queueMicrotask(() => {
          if (failure === "request" || failure === "request-without-error") {
            operationRequest.error = failure === "request"
  ...
  const database = {
    ...
    transaction: () => {
      queueMicrotask(() => {
        if (failure === "transaction-error" || failure === "transaction-error-without-error") {
          transaction.error = failure === "transaction-error"
            ? new DOMException("IndexedDB transaction failed.")
            : null;
          transaction.onerror?.(new Event("error"));
        } else if (failure === "transaction-abort" || failure === "transaction-abort-without-error") {
          transaction.error = failure === "transaction-abort"
...
function makeExistingStoreFactory(): { factory: IDBFactory; createObjectStore: jest.Mock } {
  const request = {
    onerror: null,
    onupgradeneeded: null,
    onsuccess: null,
  ...
  const transaction: FakeTransaction = {
    ...
    objectStore: () => ({
      getAll: () => {
        queueMicrotask(() => operationRequest.onsuccess?.(new Event("success")));
        return operationRequest;
      },
    }),
  };
...
describe("customGlassStorage", () => {
  ...
  it("settles successful storage operations instead of leaving requests pending", async () => {
    await expect(settleWithin(upsertPersistedCustomGlass(firstInput))).resolves.toBeUndefined();
  });

  it("does not recreate object stores that already exist during an upgrade", async () => {
    const { factory: existingStoreFactory, createObjectStore } = makeExistingStoreFactory();
  ...
  it("accepts only tabulated rows with finite numeric pairs", () => {
    ...
    expect(isPersistedCustomGlassRow({
      label: "CUSTOM",
      type: "tabulated",
      pairs: [[587.56, 1.5], [486.13, Number.NaN]],
    })).toBe(false);
  });
  ...
  it.each([
    ...
    { label: "CUSTOM", type: "tabulated", pairs: [{ length: 2, 0: 587.56, 1: 1.5 }] },
    { label: "CUSTOM", type: "tabulated", pairs: [[587.56, 1.5], [486.13, Number.NaN]] },
    { label: "CUSTOM", type: "tabulated", pairs: [{ wavelength: 587.56, index: 1.5 }] },
    { label: "CUSTOM", type: "tabulated", pairs: [[587.56, Number.POSITIVE_INFINITY]] },
  ...
  it("creates both object stores and supports single and batch upserts", async () => {
    ...
    await upsertPersistedCustomGlasses([secondInput, { ...firstInput, name: "REPLACED" }]);

    expect(await readPersistedCustomGlasses()).toEqual(expect.arrayContaining([
    ...
    expect((await readRawStore(factory, customGlassStorageConstants.quarantinedStore))).toEqual([]);
  });
  ...
  it("closes opened databases after successful storage operations", async () => {
    ...
    try {
      ...
      await quarantinePersistedCustomGlass(toPersistedCustomGlassRow(firstInput));
  ...
  it("rejects when IndexedDB is unavailable", async () => {
    ...
    try {
      await expect(readPersistedCustomGlasses()).rejects.toThrow("IndexedDB is not available.");
    } finally {
  ...
  it("rejects IndexedDB open, request, transaction-error, and transaction-abort failures", async () => {
    for (const failure of ["open", "request", "transaction-error", "transaction-abort"] as const) {
      _setIndexedDbForTest(makeFailureFactory(failure));
      await expect(settleWithin(readStoredCustomGlassRows())).rejects.toBeInstanceOf(DOMException);
    }
  });
  ...
  it("uses fallback messages when IndexedDB failures omit an error object", async () => {
    ...
    for (const [failure, message] of failures) {
      _setIndexedDbForTest(makeFailureFactory(failure));
      await expect(settleWithin(readStoredCustomGlassRows())).rejects.toThrow(message);
    }
  });
});
