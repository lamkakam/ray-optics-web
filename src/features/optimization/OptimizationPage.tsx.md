...
- The page passes the selected optimizer kind/method to the radius, thickness, and asphere modals so `variable` mode rendering and validation follow the active least-squares method.
- When the user explicitly switches the Method select to `lm` and the current config violates the SciPy `lm` dimension rule (`residuals < variables`), the page opens the existing warning modal with that specific message at switch time.
...
- That method-switch warning is targeted: the page only opens it for the `lm` residual-count mismatch, and does not surface unrelated intermediate config errors as warnings during ordinary form editing.
