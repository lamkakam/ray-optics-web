...
- `Optimize` first blurs the active editable browser element when present so AG Grid stops editing and commits the latest typed field, wavelength, or operand value before `buildOptimizationConfig()` reads from the store.
