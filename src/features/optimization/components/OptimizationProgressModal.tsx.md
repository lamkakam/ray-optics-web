...
Blocking optimization-status modal for the Optimization page. It renders an Apache ECharts line chart of optimization progress with iteration on the x-axis and `log10(total merit function value)` on the y-axis.
...
- Series data is built directly from `progress.map(({ iteration, log10_merit_function_value }) => [iteration, log10_merit_function_value])`.

## Chart Conventions

- `xAxis.name` is `Iteration`.
- `yAxis.name` is `log10(Total merit function value)`.
