...
const TYPE_OPTIONS = ["bend", "dec and return", "decenter", "reverse"].map(
...
export function TiltDecenterVarModal({
  isOpen,
  optimizationModel,
  surfaceIndex,
  decenterState,
  ...
  if (
    ...
    decenterState === undefined
  ) {
    return <Modal isOpen={false} title="Tilt & Decenter Variable / Pickup" />;
  }
  return (
    <Editor
      key={`${surfaceIndex}:${JSON.stringify(decenterState)}`}
...
function Editor({
  ...
  readonly state: DecenterOptimizationState;
  readonly canUseBounds: boolean;
  ...
  const sources = getRadiusPickupSourceSurfaceOptions(
    model.surfaces.length,
    state.surfaceIndex,
  );
  const setMode = (component: ComponentKey, mode: AsphereMode) =>
    setDraft((current) => ({ ...current, [component]: mode }));
  const invalid =
    canUseBounds &&
    COMPONENTS.some(([component, label]) => {
      const mode = draft[component];
      return (
        mode.mode === "variable" &&
        minLessThanMaxRule(label, mode.min, mode.max) !== undefined
      );
    });
  return (
    <Modal
      isOpen
      title="Tilt & Decenter Variable / Pickup"
      size="lg"
      footer={
        <div className="flex justify-end gap-3">
          ...
          <Button
            ...
            disabled={invalid}
            onClick={() => {
              onSave(state.surfaceIndex, draft);
              onClose();
            }}
          >
            Confirm
    ...
    >
      <div className="space-y-4">
        <div>
          <Label htmlFor="decenter-type">Type</Label>
          <Select
            id="decenter-type"
            aria-label="Tilt and decenter type"
            value={draft.type}
            disabled={draft.lockedType}
            options={TYPE_OPTIONS}
            onChange={(event) => {
              const type = event.target
                .value as DecenterConfig["coordinateSystemStrategy"];
        ...
        {COMPONENTS.map(([component, label]) => {
          ...
          return (
            <div key={component} className="space-y-2">
                    ...
                    setMode(
                      ...
                      next === "constant"
                        ...
                        : next === "variable"
                          ? {
                              mode: "variable",
                              min: mode.mode === "variable" ? mode.min : "0",
                              max: mode.mode === "variable" ? mode.max : "0",
                            }
                          : {
                              mode: "pickup",
                              sourceSurfaceIndex:
                                mode.mode === "pickup"
                                  ...
                                  : String(sources[0]?.value ?? ""),
                              scale: mode.mode === "pickup" ? mode.scale : "1",
                              offset:
                                mode.mode === "pickup" ? mode.offset : "0",
                            },
                    )
                  }
                />
              ...
              {mode.mode === "variable" && (
                <VariableFields.Component
                  idPrefix={`decenter-${component}`}
                  minAriaLabel={`${label} Min.`}
                  minValue={mode.min}
                  maxAriaLabel={`${label} Max.`}
                  maxValue={mode.max}
                  onMinChange={(value) =>
                    setMode(component, { ...mode, min: value })
                  }
                  onMaxChange={(value) =>
                    setMode(component, { ...mode, max: value })
                  }
                  errorText={
                    canUseBounds
                      ? minLessThanMaxRule(label, mode.min, mode.max)
                      : undefined
                  }
              ...
              {mode.mode === "pickup" && (
                <PickupModeFields
                  ...
                  sourceSurfaceAriaLabel={`${label} source surface`}
                  sourceSurfaceValue={mode.sourceSurfaceIndex}
                  sourceSurfaceOptions={sources}
                  onSourceSurfaceChange={(value) =>
                    setMode(component, { ...mode, sourceSurfaceIndex: value })
                  }
                  scaleLabel="Scale"
                  scaleAriaLabel={`${label} scale`}
                  scaleValue={mode.scale}
                  onScaleChange={(value) =>
                    setMode(component, { ...mode, scale: value })
                  }
                  offsetLabel="Offset"
                  offsetAriaLabel={`${label} offset`}
                  offsetValue={mode.offset}
                  onOffsetChange={(value) =>
                    setMode(component, { ...mode, offset: value })
                  }
                  className="ml-36 grid gap-3 pl-3"
