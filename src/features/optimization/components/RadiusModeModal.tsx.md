...
- Reuses shared helper functions from `features/optimization/lib/modalHelpers.ts` for draft seeding, mode options, keyed remount serialization, default mode transitions, and curvature-radius zero-crossing validation.
- Delegates the shared mode selector plus variable/pickup form sections to `ModeSelectField.tsx`, `VariableModeFields.tsx`, and `PickupModeFields.tsx`, while keeping radius-specific draft transitions, copy, and validation in this modal.
