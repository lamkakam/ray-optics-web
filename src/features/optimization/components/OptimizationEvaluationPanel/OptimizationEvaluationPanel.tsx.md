...
- When `rows` is empty and `invalidConfigMessage` is provided, renders that message first with the same `Paragraph` placeholder text size as the existing empty-state text and the shared error text color, followed by the existing placeholder text.
