...
import {
  BottomDrawer,
  type TabItem,
} from "@/shared/components/layout/BottomDrawer";
...
type OptimizerPatch = Parameters<
  ComponentProps<typeof OptimizationAlgorithmTab>["onChangeOptimizer"]
>[0];
type FieldsProps = Pick<ComponentProps<typeof OptimizationWeightsGrid>, "rows">;
type GridEditLifecycleProps = Pick<
  ComponentProps<typeof OptimizationWeightsGrid>,
...
export const BottomDrawerContainer = memo(function BottomDrawerContainer({
  ...
  const handleChangeOptimizer = useCallback(
    (patch: OptimizerPatch) => {
      if (
        ...
        patch.kind !== optimizationStore.getState().optimizer.kind
      ) {
      ...
      optimizationStore.setState((state) => {
        if (state.optimizer.kind === "least_squares") {
          return {
            optimizer: { ...state.optimizer, ...patch, kind: "least_squares" },
          };
        }
        if (state.optimizer.kind === "glass_expert") {
          return {
            optimizer: { ...state.optimizer, ...patch, kind: "glass_expert" },
          };
        }
        return {
          optimizer: {
  ...
  const handleOpenThicknessModal = useCallback(
    (surfaceIndex: number) => {
  ...
  const handleOpenAsphereModal = useCallback(
    (surfaceIndex: number) => {
  ...
  const handleOpenGlassModal = useCallback(
    (surfaceIndex: number) => {
      optimizationStore.getState().openGlassModal(surfaceIndex);
    },
    [optimizationStore],
  );
  const handleOpenDecenterVarModal = useCallback(
    (surfaceIndex: number) =>
      optimizationStore.getState().openDecenterVarModal(surfaceIndex),
    [optimizationStore],
  );

  const handleUpdateFieldWeight = useCallback(
  ...
  const handleUpdateWavelengthWeight = useCallback(
    (index: number, value: number) => {
  ...
  const handleDeleteOperand = useCallback(
    (id: string) => {
  ...
  const handleUpdateOperand = useCallback(
    (id: string, patch: Partial<Omit<(typeof operands)[number], "id">>) => {
  ...
  const tabs = useMemo<TabItem[]>(
    () => [
      ...
      },
      {
        ...
        content: <OptimizationLensPrescriptionGrid {...prescriptionProps} />,
      },
      {
        id: "operands",
        label: "Operands",
