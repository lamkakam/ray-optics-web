...
describe("methodCapabilities", () => {
  ...
  it("keeps the user-facing optimizer metadata and formatted defaults stable", () => {
    ...
    expect(OPTIMIZER_UI_CONFIG.least_squares.methods).toEqual([
      expect.objectContaining({ kind: "trf", label: "Trust Region Reflective", canOptimizeGlass: false }),
      expect.objectContaining({ kind: "lm", label: "Levenberg-Marquardt", canOptimizeGlass: false }),
    ]);
    expect(OPTIMIZER_UI_CONFIG.differential_evolution.numericFields[0]).toEqual(expect.objectContaining({
      kind: "max_nfev",
  ...
  it("rejects an unknown operand kind at the shared metadata boundary", () => {
    expect(() => getOptimizationOperandMetadata("unknown" as OptimizationOperandKind)).toThrow(
      "Unknown optimization operand kind: unknown",
    );
  });
});
