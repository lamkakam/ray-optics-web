...
export function interpolateAnalysisHeatmapColor(
  ...
  const palettePosition =
    clampedValue * (ANALYSIS_HEATMAP_COLOR_PALETTE.length - 1);
  const lowerIndex = Math.max(
    0,
    Math.min(
      ANALYSIS_HEATMAP_COLOR_PALETTE.length - 1,
  ...
  const [lowerRed, lowerGreen, lowerBlue] = hexToRgba(
    ANALYSIS_HEATMAP_COLOR_PALETTE[lowerIndex],
    255,
  );
  const [upperRed, upperGreen, upperBlue] = hexToRgba(
    ANALYSIS_HEATMAP_COLOR_PALETTE[upperIndex],
    255,
  );
