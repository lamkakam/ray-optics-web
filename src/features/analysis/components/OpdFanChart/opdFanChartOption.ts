...
export function buildOpdFanChartOption(
  ...
  const legendData = opdFanData.map((seriesData) =>
  ...
  const sideBySideSubplotWidth = Math.max(
    0,
    (chartWidth - OPD_FAN_GRID_LEFT - OPD_FAN_GRID_RIGHT - OPD_FAN_GRID_GAP) /
      2,
  );
  const sideBySideSubplotHeight = Math.max(
    0,
    chartHeight - gridTop - OPD_FAN_GRID_BOTTOM,
  );
  const stackedSubplotWidth = Math.max(
    0,
    chartWidth - OPD_FAN_GRID_LEFT - OPD_FAN_GRID_RIGHT,
  );
  const stackedSubplotHeight = Math.max(
    0,
    (chartHeight - gridTop - OPD_FAN_GRID_BOTTOM - OPD_FAN_STACKED_GRID_GAP) /
      2,
  );
