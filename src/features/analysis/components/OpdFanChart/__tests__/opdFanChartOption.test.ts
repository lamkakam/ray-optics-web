...
describe("buildOpdFanChartOption", () => {
  ...
  it("creates separate subplot titles and axis labels", () => {
    ...
    const yAxisLabel0 = option.yAxis[0]?.axisLabel as unknown as {
      formatter: (value: number) => string;
    };
    const yAxisLabel1 = option.yAxis[1]?.axisLabel as unknown as {
      formatter: (value: number) => string;
    };
  ...
  it("reserves extra top space for wrapped wavelength legends on narrow small screens", () => {
    const sixWavelengthOpdFanData: OpdFanData = Array.from(
      { length: 6 },
      (_, index) => ({
        fieldIdx: 0,
        wvlIdx: index,
