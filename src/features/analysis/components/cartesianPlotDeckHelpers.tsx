...
export function getInitialOrthographicZoom(
  plotSide: number,
  axisExtent: number,
): number {
...
export function useMeasuredChartSize(
...
export function getCartesianPlotLayout(size: ChartSize): CartesianPlotLayout {
  const plotSide = Math.max(
    ...
    Math.min(
      size.width -
        CARTESIAN_CHART_LEFT -
        CARTESIAN_CHART_RIGHT -
        CARTESIAN_LEGEND_WIDTH,
...
export function getVisibleAxisDomains(
  plotSide: number,
  ...
  return (
    ...
    >
      ...
      {xAxisTicks.map((tick, index) => {
        ...
        return (
          <g key={`x-${index}`}>
            ...
            />
            <text
              x={layout.plotLeft + offset}
              y={layout.plotTop + layout.plotSide + 18}
              textAnchor="middle"
              fontSize="11"
              fill="currentColor"
            >
      ...
      {yAxisTicks.map((tick, index) => {
        const offset =
          (1 - index / (CARTESIAN_TICK_COUNT - 1)) * layout.plotSide;
        return (
          <g key={`y-${index}`}>
            <line
              x1={layout.plotLeft - 4}
              y1={layout.plotTop + offset}
              x2={layout.plotLeft}
              y2={layout.plotTop + offset}
              stroke="currentColor"
            />
            <text
              x={layout.plotLeft - 10}
              y={layout.plotTop + offset + 4}
              textAnchor="end"
              fontSize="11"
              fill="currentColor"
            >
      ...
      {hasColorBar ? (
        <>
          ...
          <text
            x={colorBarLabelX}
            y={layout.plotTop + colorBarHeight}
            fontSize="11"
            fill="currentColor"
          >
            {colorBarBottomLabel}
