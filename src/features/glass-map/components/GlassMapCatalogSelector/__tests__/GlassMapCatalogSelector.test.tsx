...
describe("GlassMapCatalogSelector", () => {
  it("lists every supported catalog, including catalogs with no glasses", () => {
    ...
    expect(
      within(screen.getByRole("combobox", { name: "Catalog" }))
        .getAllByRole("option")
  ...
  it("defaults to the first catalog with an eligible glass and leaves Glass blank", () => {
    ...
    expect(screen.getByRole("combobox", { name: "Catalog" })).toHaveValue(
      "Hoya",
    );
  ...
  it.each(["", "N-", "missing"])(
    "keeps Select disabled for invalid input %p",
    async (value) => {
      renderSelector();
      await userEvent.selectOptions(
        screen.getByRole("combobox", { name: "Catalog" }),
      ...
      expect(
        screen.getByRole("button", { name: "Select glass" }),
      ).toBeDisabled();
    },
  );
  ...
  it("selects an eligible Custom glass", async () => {
    ...
    await userEvent.click(screen.getByRole("button", { name: "Select glass" }));
    expect(onSelect).toHaveBeenCalledWith(
      expect.objectContaining({ catalogName: "Custom", glassName: "My Glass" }),
    );
  });
});
