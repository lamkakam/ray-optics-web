...
export const SpecsConfiguratorStoreContext =
...
export const SpecsConfiguratorStoreProvider: React.FC<
  SpecsConfiguratorStoreProviderProps
> = ({ children }) => {
...
export const useSpecsConfiguratorStore =
  (): StoreApi<SpecsConfiguratorState> => {
    const store = useContext(SpecsConfiguratorStoreContext);
    if (store === undefined) {
      throw new Error(
        "`useSpecsConfiguratorStore` must be used within `SpecsConfiguratorStoreProvider`",
      );
    }
    return store;
  };
