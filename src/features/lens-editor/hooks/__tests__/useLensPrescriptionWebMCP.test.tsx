...
describe("useLensPrescriptionWebMCP", () => {
  it("routes executions through the latest lookup maps without re-registering tools", async () => {
    ...
    const registeredSet = registrations.find(
      (tool) => tool.name === "set_lens_prescription",
    );
    if (registeredSet === undefined)
      throw new Error("set_lens_prescription was not registered");

    rerender({ lookupMaps: customMaps });
    expect(registerTool).toHaveBeenCalledTimes(5);
    await act(async () => {
      await registeredSet.execute(prescription, {
