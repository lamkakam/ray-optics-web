...
function getInitialAsphericalType(
  asphericalRow: GridRow | undefined,
): AsphericalType {
...
function getInitialAsphericalCoefficients(
  asphericalRow: GridRow | undefined,
): number[] {
  if (asphericalRow?.kind !== "surface") {
    return [];
...
function getInitialToricSweepRadiusOfCurvature(
  asphericalRow: GridRow | undefined,
): number {
  if (asphericalRow?.kind !== "surface") {
    return 0;
...
export function LensPrescriptionContainer({
  ...
  const pendingMediumSelection = useStore(
    store,
    (s) => s.pendingMediumSelection,
  );
  const asphericalModal = useStore(store, (s) => s.asphericalModal);
  const decenterModal = useStore(store, (s) => s.decenterModal);
  const diffractionGratingModal = useStore(
    store,
    (s) => s.diffractionGratingModal,
  );
  ...
  const [formattingError, setFormattingError] = useState<string | undefined>(
    undefined,
  );
  /** Reversed rows awaiting the optional reference-surface decision. */
  const [pendingReferenceSurfaceRows, setPendingReferenceSurfaceRows] =
    useState<GridRow[] | undefined>(undefined);
  ...
  const handleRowChange = useCallback(
    (id: string, patch: Partial<GridRow>) =>
      store.getState().updateRow(id, patch),
    [store],
  );
  ...
  const handleOpenAsphericalModal = useCallback(
    (rowId: string) => store.getState().openAsphericalModal(rowId),
    [store],
  );
  const handleOpenDecenterModal = useCallback(
    (rowId: string) => store.getState().openDecenterModal(rowId),
    [store],
  );
  ...
  const handleOpenApertureModal = useCallback(
    (rowId: string) => store.getState().openApertureModal(rowId),
    [store],
  );
  const handleAddRowAfter = useCallback(
    (rowId: string) => store.getState().addRowAfter(rowId),
    [store],
  );
  const handleDeleteRow = useCallback(
    (rowId: string) => store.getState().deleteRow(rowId),
    [store],
  );
  ...
  const diffractionGratingRow = rows.find(
    (r) => r.id === diffractionGratingModal.rowId,
  );
    ...
    <div className="min-[1440px]:flex min-[1440px]:h-full min-[1440px]:flex-col">
      <div role="toolbar" aria-label="Grid toolbar" className="mb-2 flex gap-2">
        <Tooltip text="Generate a Python script" portal noTouch>
          <Button variant="secondary" onClick={() => setPythonScriptOpen(true)}>
            Export Python Script
          </Button>
        </Tooltip>
        <Tooltip text="Format selected prescription rows" portal noTouch>
          <Button variant="secondary" onClick={() => setFormattingOpen(true)}>
            Formatting
          </Button>
        </Tooltip>
      </div>
      ...
      <div className="mt-2 mb-2 flex items-center gap-2">
        <Label htmlFor="auto-aperture-toggle">
          Set auto aperture dimensions:
        </Label>
        <Tooltip
          text={
            autoAperture
              ? "Turn off auto aperture dimensions"
              : "Turn on auto aperture dimensions"
          }
          noTouch
        >
          <Switch
            ...
            onCheckedChange={(checked) =>
              store.getState().setAutoAperture(checked)
            }
      ...
      <MediumSelectorModal
        ...
        initialMedium={
          mediumRow?.kind === "surface" || mediumRow?.kind === "object"
            ? mediumRow.medium
            : "air"
        }
        initialManufacturer={
          mediumRow?.kind === "surface" || mediumRow?.kind === "object"
            ? mediumRow.manufacturer
            : ""
        }
        allowReflective={!isObjectMediumRow}
        selectedMedium={pendingMediumSelection?.medium}
        selectedManufacturer={
          pendingMediumSelection?.manufacturer === ""
            ? "Special"
            : pendingMediumSelection?.manufacturer
        }
        ...
        onConfirm={(medium, manufacturer) => {
          store
            .getState()
            .commitPendingMediumSelection({ medium, manufacturer });
        }}
      ...
      <AsphericalModal
        ...
        initialConicConstant={
          asphericalRow?.kind === "surface"
            ? (asphericalRow.aspherical?.conicConstant ?? 0)
            : 0
        }
        initialType={getInitialAsphericalType(asphericalRow)}
        initialCoefficients={getInitialAsphericalCoefficients(asphericalRow)}
        initialToricSweepRadiusOfCurvature={getInitialToricSweepRadiusOfCurvature(
          asphericalRow,
        )}
        onConfirm={(params: {
                ...
                : params.type === "XToroid"
                  ? {
                      ...
                      toricSweepRadiusOfCurvature:
                        params.toricSweepRadiusOfCurvature,
                      polynomialCoefficients: params.polynomialCoefficients,
                    }
                  : params.type === "YToroid"
                    ? {
        ...
        onRemove={() => {
          store
            .getState()
            .updateRow(asphericalModal.rowId, { aspherical: undefined });
          store.getState().closeAsphericalModal();
        }}
      />
      ...
      <DecenterModal
        ...
        initialDecenter={
          decenterRow?.kind !== "object" ? decenterRow?.decenter : undefined
        }
        ...
        onRemove={() => {
          store
            .getState()
            .updateRow(decenterModal.rowId, { decenter: undefined });
          store.getState().closeDecenterModal();
        }}
      />
      ...
      <DiffractionGratingModal
        key={
          diffractionGratingModal.open
            ? diffractionGratingModal.rowId
            : "diffraction-grating-closed"
        }
      ...
      <ApertureModal
        ...
        semiDiameter={
          apertureRow?.kind === "surface" ? apertureRow.semiDiameter : 1
        }
        initialClearAperture={
          apertureRow?.kind === "surface"
            ? apertureRow.clear_aperture
            : undefined
        }
        initialEdgeAperture={
          apertureRow?.kind === "surface"
            ? apertureRow.edge_aperture
            : undefined
        }
      ...
      {formattingOpen && (
        <FormattingModal
          ...
          onConfirm={({ mode, rows: updatedRows }) => {
            if (
              mode === "reverse" &&
              firstSurfaceNeedsReferenceSurface(updatedRows)
            ) {
        ...
        onConfirm={() => {
          if (pendingReferenceSurfaceRows !== undefined) {
            store
              .getState()
              .setRows(
                insertReferenceSurfaceAfterObject(pendingReferenceSurfaceRows),
              );
          }
          setPendingReferenceSurfaceRows(undefined);
        }}
