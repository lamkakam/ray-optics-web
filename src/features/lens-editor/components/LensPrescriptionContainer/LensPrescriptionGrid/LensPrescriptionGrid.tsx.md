...
- The `Index`, `Surface`, `Radius of Curvature`, `Thickness`, `Medium`, and `Semi-diam.` initial widths come from `LENS_PRESCRIPTION_GRID_COLUMN_WIDTHS` in `shared/lib/lens-prescription-grid`; the leading row action column remains editor-specific at `100px`.
