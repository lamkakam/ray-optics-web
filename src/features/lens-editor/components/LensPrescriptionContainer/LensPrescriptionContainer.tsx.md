...
- The visible `Set auto semi-diameter:` label is paired with an Auto/Manual switch that updates `autoAperture` in the store, passes `semiDiameterReadonly` to the grid, and passes `autoAperture` to `ApertureModal` for Clear Rectangular ratio labels.
