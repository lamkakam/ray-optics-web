...
async function openObjectMediumSelector(
...
test("returns with a newly selected glass and commits it only after confirmation", async ({
  ...
  const objectRow = await selectSchottNbk7AndOpenGlassMap(page);
  const unselectedPoint = page
    .locator('[data-testid="glass-point"][stroke="none"]')
    .first();
  ...
  const selectedCatalog = await detailPanel
    .locator("span")
    .first()
    .textContent();
  ...
  const modal = page.getByRole("dialog", { name: "Select Medium" });
  await expect(modal).toBeVisible();
  await expect(modal.getByLabel("Catalog")).toHaveValue(
    selectedCatalog?.trim(),
  );
  await expect(modal.getByLabel("Glass", { exact: true })).toHaveValue(
    selectedGlass?.trim(),
  );
