...
test("import lens-config.json and verify System Specs and Prescription", async ({
  ...
  // 5 rows exist
  await expect(page.locator(`${wlGrid} .ag-row[row-index="4"]`)).toBeAttached({
    timeout: 3_000,
  });
  ...
  const asphModal = page.locator(
    '[role="dialog"][aria-labelledby="aspherical-modal-title"]',
  );
  ...
  await row1.hover();
  await (
    await getPrescriptionCell(page, prescGrid, 1, "Tilt & Decenter")
  ).click();
  const decModal = page.locator(
