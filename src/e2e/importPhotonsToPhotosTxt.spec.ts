...
const photonsToPhotosDir = path.join(
  __dirname,
  "..",
  "__tests__",
  "data",
...
test("import prime Photons to Photos txt and verify specs and prescription", async ({
  ...
  const [fileChooser] = await Promise.all([
    ...
    page
      .locator('button[aria-label="Import a file from Photons to Photos"]')
      .click(),
  ]);
  await fileChooser.setFiles(
    path.join(photonsToPhotosDir, "prime-no-glass-type.txt"),
  );
  ...
  await expect(
    page.locator(
      `${prescGrid} .ag-row[row-index="5"] .ag-cell[col-id="${colIdSurface}"]`,
    ),
  ).toContainText("Stop");
  await expect(
    page.locator(
      `${prescGrid} .ag-row[row-index="5"] .ag-cell[col-id="${colIdThickness}"]`,
    ),
  ).toContainText("4.16");
  await expect(
    page.locator(
      `${prescGrid} .ag-row[row-index="9"] .ag-cell[col-id="${colIdThickness}"]`,
    ),
