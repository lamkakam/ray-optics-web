...
**Live demo on GitHub Page:** [https://lamkakam.github.io/ray-optics-web/](https://lamkakam.github.io/ray-optics-web/)

> **Note:** This webapp requires to have HTTP COOP header set with `same-origin` and COEP header with `require-corp` for the functionality of aborting long optical system optimization.
...
# Refresh the tracked npm and Python third-party dependency license reports
...
Stryker copies those fixtures, generated helpers, tests, mocks, and Python sources into its sandbox even though some are Git-ignored. Mutation targets exclude tests/specs, test and mock directories, fixtures, E2E tests, Python, and generated code. Sandbox copying excludes local agent metadata, build artifacts, Python environments, caches, and reports. Temporary `.stryker-tmp/` directories are Git-ignored and excluded from ordinary Jest discovery and TypeScript checking.
